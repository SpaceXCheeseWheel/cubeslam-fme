...
// TODO: This page will probaby require a wholesale rewrite. Webpack does NOT like this.
...
Loading.leave = function(ctx, next){
  ...
  if( ctx.query.renderer == 'css' ){
    ...
    this.loader.push(function(done){
      ...
      loadScript('/public/javascript/renderer-css'+ctx.ext,function(err){
    ...
    this.loader.push(function(done){
      ...
      var pending = 3;
      loadScript('/public/javascript/libs/tween-max.min.js',check)
      loadScript('/public/javascript/libs/three'+ctx.ext,check)
      loadScript('/dist/renderer'+ctx.ext,check)
      function check(err){
        if( err ){return done(err)}
        --pending || init()
      }
